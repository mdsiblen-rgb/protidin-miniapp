...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 1. Approve / Decline Button - FIXED
    if (body.callback_query) {
      ...
      // FIX: UID তে _ থাকলেও যাতে না ভাঙে
      const first = data.indexOf("_");
      const last = data.lastIndexOf("_");
      const action = data.slice(0, first); // approve or decline
      const targetUid = data.slice(first + 1, last); // মাঝের পুরা UID
      const amount = data.slice(last + 1); // শেষের amount
      ...
      if (action === "approve") {
        ...
        // বাটনের মেসেজটা এডিট করে দেবে যাতে বোঝা যায় Done
        await fetch(`https://api.telegram.org/bot${token}/editMessageText`, {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ chat_id: ADMIN_ID, message_id: cb.message.message_id, text: cb.message.text + `\n\n✅ APPROVED ${amount} Tk`, reply_markup: { inline_keyboard: [] } })
        });
      } else {
        await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
          method: "POST", headers: { "Content-Type": "application/json" },
        ...
        await fetch(`https://api.telegram.org/bot${token}/editMessageText`, {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ chat_id: ADMIN_ID, message_id: cb.message.message_id, text: cb.message.text + `\n\n❌ DECLINED`, reply_markup: { inline_keyboard: [] } })
        });
      }
