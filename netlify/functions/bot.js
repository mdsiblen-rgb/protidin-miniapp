...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (body.callback_query) {
      ...
      // Firebase Status Update - এটাই App এ Approved দেখাবে জান
      try {
      ...
      if (action === "approve") {
        ...
        await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
          ...
          body: JSON.stringify({
            chat_id: ADMIN_ID,
            text: `✅ Done জান! UID: ${targetUid} এর ${amount} Tk Approve করা হলো! Firebase ও Update হলো!`
          })
        });
      } else {
        await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
    ...
    // ===== 2. WEB APP থেকে WITHDRAW আসলে - PENDING PROCESSING =====
