// FINAL FIXED FILE - Protidin Earning BD - By Jan - Firebase Added
exports.handler = async (event) => {
  ...
  try {
    ...
    if (body.callback_query) {
      ...
      // 🔥 এটাই আসল কাজ জান - Firebase Update
      try {
      ...
      if (action === "approve") {
        await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
          ...
          body: JSON.stringify({ chat_id: targetUid, text: `✅ অভিনন্দন জান! আপনার ${amount} Tk Approve হয়েছে! 5-10 মিনিটে পেমেন্ট পাবেন!` })
        });
        await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
          method: "POST", headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ chat_id: ADMIN_ID, text: `✅ Done জান! UID: ${targetUid} এর ${amount} Tk Approve হলো! Firebase ও Update হলো!` })
        });
      } else {
        await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
          method: "POST", headers: { "Content-Type": "application/json" },
    ...
    if (body.action === "withdraw") {
      ...
      await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ chat_id: uid, text: `⏳ জান আপনার ${amount} Tk Pending Processing এ আছে! Admin Approve করলে পাবেন!` })
      });
